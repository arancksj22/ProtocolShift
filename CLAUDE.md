...
- `run_trials.py`: campaign runner — N trials per (suite × backend × concurrency × payload) cell, flushes DBs between trials, captures `environment.json`, optionally samples `docker stats` and `pg_stat_statements` (`--profile`). Results go to `results/<run_id>/` (committed to git since the AWS campaign — no longer gitignored). Three flush modes: `docker` (default, `docker compose exec`, no DB ports needed), `direct` (connects to DBs over the network via `REDIS_URL`/`POSTGRES_DSN`/`MONGO_URI` or `--host` defaults — for two-node/cloud setups; server must publish DB ports via `docker-compose.expose-dbs.yml` overlay), `none` (data accumulates — this caused the Redis OOM crash in the first AWS campaign, see HOW_TO_RUN_2.md).
- `analyze.py`: trial-level 95% t-CIs, CoV, Mann-Whitney U (pooled latencies), matplotlib plots, and `migration_decision.md` — a 3-gate Phase-Gate model (p99 improvement ≥ X%, disjoint CIs + MWU p<0.01, DB-time share < Y% from pg_stat). CIs are computed across trials, not within (per-request samples are autocorrelated). Cells where either side has <2 completed trials are marked EXCLUDED in comparisons/migration tables and dropped from plots — a lone surviving trial usually measured a failing system (this rule exists because a dying Redis produced a misleading −448% row in the AWS run). Runs can be merged by copying `raw/*.json` from multiple runs into one folder and re-analyzing (see HOW_TO_RUN_2.md Part B4).
...
- **The PostgreSQL anomaly reversed under correct load generation.** The paper originally reported gRPC's p99 *worse* than REST on PostgreSQL (local 130 vs 104 ms; cloud 861 vs 347 ms) — but those numbers came from grpcurl loops (new process + new HTTP/2 connection per request). The July 2026 AWS campaign with the Python harness (persistent channel) shows gRPC 88–92% *faster* than REST on postgres at p99 under load (`results/20260718-071036/`). The paper narrative must present the anomaly as a load-generation artifact, not a protocol property.
- The AWS campaign (2× t3.medium, ap-south-1, two-node) lost 14/90 Redis trials to a Redis OOM crash during 10KB sweeps caused by `--skip-flush` accumulation; the `grpc-redis c50/10KB` single-trial cell (p99 4,354 ms) is contaminated and must never be cited as a protocol comparison. `--flush-mode direct` + `docker-compose.expose-dbs.yml` exist to prevent a recurrence; HOW_TO_RUN_2.md is the rerun guide.
- Analysis centers on p95/p99 tail latency, not means; CIs/MWU tables from `analyze.py` are the primary evidence artifact (Grafana overlays are secondary/visual).
